...
class PreTrainedTokenizer(object):
    ...
    def tokenize_xs(self, text, **kwargs):
        """ Converts a string in a sequence of tokens (string), using the tokenizer.
            Split in words for word-based vocabulary or sub-words for sub-word-based
            vocabularies (BPE/SentencePieces/WordPieces).
        ...
        def split_on_tokens(tok_list, text):
            if not text:
                return []
            if not tok_list:
                return self._tokenize_xs(text, **kwargs)
            ...
            text_idx = 0
            w_idx_every_token_ = []
            # here token is actually a sentence
            for token in tokenized_text:
                if token not in self.added_tokens_encoder and token not in self.all_special_tokens:
                    tokens, w_idx_every_token = self._tokenize_xs(token, **kwargs) # get not only the tokens but the word index for every token
                    tokenized_text_.extend(tokens)
                    shift = w_idx_every_token_[-1][-1] + 1 if len(w_idx_every_token_) > 0 else 0
                    for tmp_t in w_idx_every_token:
                        w_idx_every_token_.append([i + shift for i in tmp_t])
                else:
                    tokenized_text_.extend([token])
                    shift = w_idx_every_token_[-1] + 1
                    w_idx_every_token_.append([shift])
                text_idx += 1
            
            return tokenized_text_, w_idx_every_token_
        added_tokens = list(self.added_tokens_encoder.keys()) + self.all_special_tokens
        tokenized_text, word_index_every_token = split_on_tokens(added_tokens, text)
        return tokenized_text, word_index_every_token

    def _tokenize_xs(self, text, **kwargs):
        """ Converts a string in a sequence of tokens (string), using the tokenizer.
            Split in words for word-based vocabulary or sub-words for sub-word-based
            vocabularies (BPE/SentencePieces/WordPieces).
    ...
    def encode_xs(self,
        ...
        Args:
            ...
            return_tensors: (optional) can be set to 'tf' or 'pt' to return respectively TensorFlow tf.constant
                or PyTorch torch.Tensor instead of a list of python integers.
            first_text: the words provided by prior knowledge
            second_text: the words for the text_pair
            **kwargs: passed to the `self.tokenize()` method
        """

        def get_input_ids(text, origin_words=None):
            if isinstance(text, six.string_types):
                tokens, word_idx_every_token = self.tokenize_xs(text, origin_words=origin_words, **kwargs)
                return self.convert_tokens_to_ids(tokens), word_idx_every_token
            else:
                raise ValueError("Input is not valid. Should be a string")
        
        first_ids, first_w_idx = get_input_ids(text, first_text)
        if text_pair is not None:
            second_ids, second_w_idx = get_input_ids(text_pair, second_text)
        else:
            second_ids, second_w_idx = None, None
        ...
        return self.prepare_for_model(first_ids,
                                      ...
                                      truncation_strategy=truncation_strategy,
                                      return_tensors=return_tensors,
                                      first_w_idx=first_w_idx,
                                      second_w_idx=second_w_idx)
    ...
    def prepare_for_model(self, ids, pair_ids=None, max_length=None, add_special_tokens=True, stride=0,
                          ...
                          first_w_idx=None, second_w_idx=None):
        """
        ...
        Args:
            ...
            return_tensors: (optional) can be set to 'tf' or 'pt' to return respectively TensorFlow tf.constant
                or PyTorch torch.Tensor instead of a list of python integers.
            first_w_idx: the word index for every token in the first text
            second_w_idx: the word index for every token in the second text
        ...
        if max_length and total_len > max_length:
            ids, pair_ids, overflowing_tokens, first_w_idx, second_w_idx = self.truncate_sequences(
                                                                        ids, pair_ids=pair_ids,
                                                                        first_word_idx=first_w_idx, 
                                                                        second_word_idx=second_w_idx,
        ...
        if add_special_tokens:
            ...
            if first_w_idx != None:
                first_w_idx, second_w_idx = self.build_word_indices_with_special_tokens(first_w_idx, second_w_idx)
        ...
        if return_tensors == 'tf' and is_tf_available():
            ...
            first_w_idx = tf.constant([first_w_idx]) if first_w_idx != None else None
            second_w_idx = tf.constant([second_w_idx]) if second_w_idx != None else None
        elif return_tensors == 'pt' and is_torch_available():
            sequence = torch.tensor([sequence])
            token_type_ids = torch.tensor([token_type_ids])
            first_w_idx = torch.tensor([first_w_idx]) if first_w_idx != None else None
            second_w_idx = torch.tensor([second_w_idx]) if second_w_idx != None else None
        ...
        if first_w_idx != None:
            encoded_inputs["first_word_idx"] = first_w_idx
        if second_w_idx != None:
            encoded_inputs["second_word_idx"] = second_w_idx

        if max_length and len(encoded_inputs["input_ids"]) > max_length:
            encoded_inputs["input_ids"] = encoded_inputs["input_ids"][:max_length]
            encoded_inputs["token_type_ids"] = encoded_inputs["token_type_ids"][:max_length]
            encoded_inputs["special_tokens_mask"] = encoded_inputs["special_tokens_mask"][:max_length]
            if len(encoded_inputs["first_word_idx"]) > max_length:
                encoded_inputs["first_word_idx"] = encoded_inputs["first_word_idx"][:max_length]
                encoded_inputs["second_word_idx"] = []
            elif len(encoded_inputs["first_word_idx"]) + len(encoded_inputs["second_word_idx"]) > max_length:
                encoded_inputs["second_word_idx"] = encoded_inputs["second_word_idx"][:max_length-len(encoded_inputs["first_word_idx"])]
    ...
    def truncate_sequences(self, ids, pair_ids=None, first_word_idx=None, second_word_idx=None,
                           num_tokens_to_remove=0, truncation_strategy='longest_first', stride=0):
        ...
        if num_tokens_to_remove <= 0:
            return ids, pair_ids, [], first_word_idx, second_word_idx
        ...
        if truncation_strategy == 'longest_first':
            ...
            for _ in range(num_tokens_to_remove):
                if pair_ids is None or len(ids) > len(pair_ids):
                    ...
                    ids = ids[:-1]
                    first_word_idx = first_word_idx[:-1] if first_word_idx != None else None
                else:
                    pair_ids = pair_ids[:-1]
                    second_word_idx = second_word_idx[:-1] if second_word_idx != None else None
            ...
            first_word_idx = first_word_idx[:-num_tokens_to_remove] if first_word_idx != None else None
        elif truncation_strategy == 'only_second':
            assert pair_ids is not None and len(pair_ids) > num_tokens_to_remove
            window_len = min(len(pair_ids), stride + num_tokens_to_remove)
            overflowing_tokens = pair_ids[-window_len:]
            pair_ids = pair_ids[:-num_tokens_to_remove]
            second_word_idx = second_word_idx[:-num_tokens_to_remove] if second_word_idx != None else None
        ...
        return (ids, pair_ids, overflowing_tokens, first_word_idx, second_word_idx)
    ...
    def build_word_indices_with_special_tokens(self, word_idx_0, word_idx_1=None):
        """
        Build model inputs from a sequence or a pair of sequence for sequence classification tasks
        by concatenating and adding special tokens.
