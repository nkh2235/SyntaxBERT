...
"""Prior knowledge Masking BERT model. """
...
class PMBertSelfAttention(nn.Module):
    def __init__(self, config):
        super(PMBertSelfAttention, self).__init__()
        ...
        # task
        self.use_task_aggregation = True
        if self.use_task_aggregation:
            #self.query_task = torch.randn((1, self.all_head_size), requires_grad=True, device=self.query.weight.device)
...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        self.self = PMBertSelfAttention(config)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
        ...
        # Syntax BERT attention_mask.dim() == 4
        else:
